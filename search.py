...
def depthFirstSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        # 弹栈（弹出的节点是刚压入的节点），将栈顶的节点及到达该节点的行为赋给当前节点及行为
        cur_node, actions = fringe.pop()

        # 如果当前节点到达目标位置，返回行为链表，结束搜索
        ...
        if cur_node not in closed:
            ...
            # 循环查找后继节点的状态（包括位置，方向和代价），如果位置不在走过的节点中，则将当前节点路径加上此次方向压入栈中
...
def breadthFirstSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        # 弹队列（弹出的节点是最初压入的节点），将栈顶的节点及到达该节点的行为赋给当前节点及行为
        cur_node, actions = fringe.pop()
        ...
        if cur_node not in closed:
            ...
            for location, direction, cost in expand:
                # 如果后继节点不在走过路程中，则将当前节点位置，路径加上此次方向压入队列中
                if (location not in closed):
                    fringe.push((location, actions + [direction]))
    ...
    while fringe:
        ...
        if currState not in visited:
            ...
            for successor, action, cost in successors:
                ...
                nextCost = problem.getCostOfActions(tempActions)  # 计算代价
